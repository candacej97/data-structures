...
/**
 * This class should be used to create a Tree object. Using the correct parameters, the object
 * should be created. If parameters are wrong, exceptions will be thrown
 * 
 ...
 * 
 * The variables are private, but there are getters so that the user/another class can find out 
...
public class Tree implements Comparable<Tree>{
	...
	protected Tree(int tree_id, int tree_dbh, String status, String health, String spc_name, 
		...
		try{setTreeID(tree_id);}
		catch(IllegalArgumentException e){}
		
		try{setTreeDBH(tree_dbh);}
		catch(IllegalArgumentException e){}

		try{setTreeStatus(status);}
		catch(IllegalArgumentException e){}
		
		try{setTreeHealth(health);}
		catch(IllegalArgumentException e){}
	
		try{setSpecies(spc_name);}
		catch(IllegalArgumentException e){}
	
		try{setZip(zipcode);}
		catch(IllegalArgumentException e){}
	
		try{setBoro(boroname);}
		catch(IllegalArgumentException e){}
	 ...
	 * @throws IllegalArgumentException
	 * */
	private void setSpecies(String spc_name) throws IllegalArgumentException{
		if(spc_name != null && spc_name instanceof String){
	...
	private void setZip(int zipcode) throws IllegalArgumentException{
		if(zipcode > 99999){throw new IllegalArgumentException();}	
	...
	/**public static void main(String[] args){
		//-----TESTING ENVIRONMENT------
		Tree One = new Tree (180683, 3, "alive", "fair", "red maple", 375, "Queens", 1027431.148, 202756.7687);
		System.out.printf("Tree One's ID # is: %d.%nIt's DBH is: %d.%nIt's species "
				+ "name is: %s. It's zipcode is: %05d. It's borough is: %s.%nIt's "
				+ "status is: %s, health: %s%nx_sp: %f, and y_sp: %f.", One.getTreeID(),
				One.getTreeDBH(), One.getSpecies(), One.getZipcode(), One.getBoroname(), One.getTreeStatus(),
				One.getTreeHealth(), One.getX_sp(), One.getY_sp());
		System.out.println();
		System.out.println();
		System.out.println(One.toString());

		//test equals()
		Tree Two = new Tree(192755,21,"Alive","Fair","London planetree",11223,"Brooklyn",992653.7253,152903.6306);
