...
/**
 * This class opens and reads data files, parses the lines within it, creates objects and uses those objects
 * to give back specific information depending on the user's input of a species name. 
 * 
 * 
 * 
...
public class NYCStreetTrees {
	...
	static String file;
	static TreeList nycTrees = new TreeList();

	/** 
	 * Sets the global variable file as the constructors parameter
	 * 
	 * @param file_path
	 * @throws FileNotFoundException, FileCannotBeRead
	 */
	NYCStreetTrees(String file_path) throws FileNotFoundException, FileCannotBeRead{
		this.file = file_path;	
	}
	...
	/**
	 * Method will create a Tree object by parsing all the strings from ArrayList param
	 * 
	 * @param a
	 * @return tree
	 */
	...
	/**
	 * Method takes all inputs needed and formats them into a string
	 * @param input
	 * @return info
	 */
	...
	public static void main(String[] args) throws SpeciesNotFoundException, FileCannotBeRead, FileNotFoundException {
			...
			if(file.exists()){
				if(file.canRead()){
					...
					while(fileIO.hasNextLine()){
						...
						//put the line into a Tree object & add the Tree to the TreeList objects arraylist
						Tree t = createTree(lineArray);
						NYCStreetTrees.nycTrees.listOfAllTrees.add(t);
					}
					fileIO.close();
				}
				...
				while(!input.equalsIgnoreCase("quit")){
					...
					try{
						if(NYCStreetTrees.nycTrees.getCountByTreeSpecies(input) > 0){
							//print out the information we need
							String speciesMatches = "";
							for(String s : NYCStreetTrees.nycTrees.getMatchingSpecies(input)){
								speciesMatches += "\t" + s + "\n";
							}
							System.out.printf("All the species found: \n%s\n", speciesMatches);
							System.out.println(NYCStreetTrees.getInfo(input));
						}
						else if(input.equalsIgnoreCase("quit")){break;}
						else{throw new SpeciesNotFoundException();}
					}
					catch(SpeciesNotFoundException s){continue;}
				}//end of while loop
				scanner.close();
		...
		catch(FileNotFoundException f){System.err.println("Error: The file you provided was "
				+ "not found.");}
		catch(FileCannotBeRead f){System.err.println("Error: The file you provided was "
				+ "cannot be read.");}
